...
/**
 * First server-side bridge for UX2B. It accepts a session only after the OIDC
 * adapter has cryptographically verified it. It then resolves durable
 * membership by the session tenant/subject and snapshots authority for exactly
 * this request. No actor/tenant/roles/permissions are accepted from a browser
 * command payload.
 */

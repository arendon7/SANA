...
export interface BootstrapIdentityWiring {
  verifyConnectivity():Promise<IdentityConnectivityEvidence>;
}
export interface BootstrapPostgresWiring {
  verifyConnectivity():Promise<PostgresConnectivityEvidence>;
}
export interface BootstrapExternalAckWiring {
  verifyConnectivity():Promise<ExternalAckConnectivityEvidence>;
}
...
export type ProductionBootstrapPreflightResult=Readonly<{
  ...
  state:'BLOCKED_PRODUCTION_PREREQUISITES'|'READY_FOR_D10_HUMAN_REVIEW'|'READY_FOR_EXPLICIT_ACTIVATION_REVIEW';
...
export function resolveControlProductionReleaseCandidate(env:ControlProductionBootstrapEnvironment):ControlReleaseCandidate {
  ...
  const candidate=Object.freeze({
    version:required(env,'AGROWAY_CONTROL_RELEASE_VERSION'),
    headSha:required(env,'AGROWAY_CONTROL_GIT_HEAD_SHA'),
    reviewBundleSha256:required(env,'AGROWAY_CONTROL_REVIEW_BUNDLE_SHA256')
  });
...
function buildBindingEvidence(candidate:ControlReleaseCandidate,observedAt:string):ProductionBindingEvidence {
  const material={
    ...
    candidate,
    identityProviderBound:true as const,
    identityProviderConnectivityCertified:true as const,
    postgresSecretsBound:true as const,
    postgresConnectivityCertified:true as const,
    externalAckProviderBound:true as const,
    externalAckProviderConnectivityCertified:true as const,
    observedAt
  };
  return Object.freeze({...material,evidenceDigestSha256:computeProductionBindingEvidenceDigest(material)});
}

export function createControlProductionBootstrap(
  env:ControlProductionBootstrapEnvironment,
  factories:ControlProductionBootstrapFactories,
  now:()=>Date=()=>new Date()
):ControlProductionBootstrap {
  const candidate=resolveControlProductionReleaseCandidate(env);
  let identity:BootstrapIdentityWiring;
  let postgres:BootstrapPostgresWiring;
  let externalAck:BootstrapExternalAckWiring;
  try {
    identity=factories.createIdentity(env);
    postgres=factories.createPostgres(env);
    externalAck=factories.createExternalAck(env);
  } catch(error) {
    throw new Error('PRODUCTION_BOOTSTRAP_WIRING_CREATION_FAILED',{cause:error});
  }

  let latestReadiness:ProductionReadinessAssessment|undefined;
  let latestBindings:ProductionBindingEvidence|undefined;
  ...
  async function preflight(d10?:D10HumanProductApproval):Promise<ProductionBootstrapPreflightResult> {
    ...
    latestBindings=undefined;
    const assessed=await readiness.assess(d10);
    latestReadiness=assessed;
    ...
    const state=assessed.state==='READY_FOR_EXPLICIT_ACTIVATION_REVIEW'
      ? 'READY_FOR_EXPLICIT_ACTIVATION_REVIEW'
      : assessed.state==='READY_FOR_D10_HUMAN_REVIEW'
        ? 'READY_FOR_D10_HUMAN_REVIEW'
        : 'BLOCKED_PRODUCTION_PREREQUISITES';
  ...
  return Object.freeze({
    protocol:CONTROL_PRODUCTION_BOOTSTRAP_PROTOCOL,
    state:'CONFIGURED_FAIL_CLOSED' as const,
    candidate,
    productionExecutionAvailable:false as const,
    browserActivationAllowed:false as const,
    get realBindingEvidenceIssued(){return latestBindings!==undefined;},
    preflight,
    authorizeActivation
  });
}

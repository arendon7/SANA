(() => {
  ...
  const INTEGRITY='SNAPSHOT_COMMERCIAL_REFERENCES_ONLY · CONTENT_MINIMIZED · NO_LIVE_FALLBACK · ROW_LEVEL_STRUCTURAL_DIFF · SEMANTIC_GRANULARITY_CHANGE ≠ REFERENCE_CHANGE · LEGACY_REFERENCE_NOT_CAPTURED ≠ INVALID · REFERENCE_CHANGE ≠ COMMERCIAL_EXECUTION ≠ VERIFIED_CONTRACT ≠ PAYMENT_EXECUTION ≠ GUARANTEED_REVENUE ≠ CREDIT_RISK ≠ INVESTMENT_SIGNAL';
  ...
  function diff(a,b){
    ...
    if(!da||!db)return {valid:true,state:'PARTIAL_GRANULARITY',changes:[],total:0,rowChanges:0,semanticChanges:0,integrity:INTEGRITY};
    ...
    return {valid:true,state:'CAPTURED_BOTH',changes,total:changes.length,rowChanges:changes.filter(c=>c.field==='referenceRow').length,semanticChanges:changes.filter(c=>c.changeKind==='SEMANTIC_GRANULARITY_CHANGED').length,comparisonFields:rowFields.slice(),integrity:INTEGRITY};
  }
  function panel(){

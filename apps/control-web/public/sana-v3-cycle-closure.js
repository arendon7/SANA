(() => {
  ...
  function economicRecords(lot){return storage.records.filter(r=>r.type==='economics-cost'&&(r.lot===lot||r.values?.lot===lot))}
  ...
  function dossier(plan){
    ...
    const econ=economicRecords(plan.lot);
    const econSupported=econ.filter(r=>r.values?.evidence&&r.values.evidence!=='Sin soporte');
    ...
    const gates={
      ...
      economics:econ.length>0&&econSupported.length>0,
    ...
    return {plan,lot,activities,relations,required,evidenceReady,evidenceGaps,resolved,open,legacy,result,incidents,openIncidents,econ,econSupported,sources,impact,reviews,inventoryLinks,worklogLinks,actualEvidence,gates,completeness,readyForArchive};
  }

...
function focusBoundEntity(ctx){const stage=pathStage();const id=stage==='EXCEPTION'?ctx?.exceptionId:stage==='EVIDENCE'?ctx?.evidenceIds?.[0]:stage==='APPROVAL'||stage==='HANDOFF'?ctx?.proposalId:null;if(!id)return;for(const delay of [60,220])setTimeout(()=>{const el=document.querySelector(`[data-id="${CSS.escape(id)}"]`);if(el&&!el.classList.contains('active'))el.click()},delay)}
document.addEventListener('click',e=>{const card=e.target.closest('[data-id]');if(card&&RELATIONS[card.dataset.id])bind(card.dataset.id);if(e.target.closest('#prepareBtn,#exportBtn,#ackBtn'))setTimeout(()=>write(read()||{}),180)},{capture:true});
const ctx=hydrateFromQuery();renderFlow(ctx);focusBoundEntity(ctx);

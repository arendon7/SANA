(() => {
  ...
  function chainFor(lot){
    ...
    records.filter(r=>!['structured-visit','plan-review','harvest-result','plan-activity','activity-evidence','activity-close'].includes(r.type)).forEach(r=>proofs.push({title:r.title,meta:`${r.type} · ${r.createdAt||'Ahora'}`,state:'LOCAL_ONLY',quality:'Sandbox'}));
    const results=resultRows(lot,records);

    return {unit,plans,tasks,evidence,records,workflowActivities,identity:identityRows,planning,field,execution,proofs,results};
  }
  ...
  function passportOperational(){
    ...
    const nextGap=pendingTasks?'Registrar evidencia realmente aportada para actividades que la exigen':legacyClosures?'Reconstruir cierres previos que no tienen contrato de evidencia':!c.field.length?'Registrar acompañamiento técnico estructurado':!c.results.length?'Registrar resultado/cierre productivo':'Mantener continuidad entre plan, campo, evidencia y resultado';

    return `${head('SANA · PASSPORT Y EVIDENCIA','Reconstruir por qué ocurrió cada decisión y qué resultado siguió.','Passport conecta origen, plan versionado, actividad, ejecución, evidencia realmente aportada y resultado productivo. La expectativa de evidencia nunca se contabiliza como prueba.',`<button class="btn secondary" data-action="exportPassport">Exportar vista DEMO</button>`)}

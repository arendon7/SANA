(() => {
  ...
  function chainFor(lot){
    ...
    const field=records.filter(r=>r.type==='structured-visit').map(r=>({title:r.values?.purpose||r.title,meta:`${r.values?.owner||identity?.displayName||'Responsable'} · ${r.values?.finding||'sin hallazgo'}`,state:'LOCAL_ONLY',quality:'Estructurado'}));
    ...
    records.filter(r=>r.type!=='structured-visit'&&r.type!=='plan-review').forEach(r=>proofs.push({title:r.title,meta:`${r.type} · ${r.createdAt||'Ahora'}`,state:'LOCAL_ONLY',quality:'Sandbox'}));

    return {unit,plans,tasks,evidence,records,identity,planning,field,execution,proofs};
  }
  ...
  function integrityScore(chain){
    ...
    return Math.round(plan*.25+taskEvidence*.25+proof*.3+contextual*.2);
  }
  ...
  function passportOperational(){
    ...
    const nextGap=pendingTasks?'Completar evidencia de actividades sin prueba vinculada':!c.field.length?'Registrar acompañamiento técnico estructurado':'Mantener continuidad entre plan, campo y evidencia';

    return `${head('SANA · PASSPORT Y EVIDENCIA','Reconstruir por qué ocurrió cada decisión.','Passport conecta origen, plan, acompañamiento, ejecución y pruebas. No aumenta autoridad: hace visible la procedencia y permite detectar vacíos antes de afirmar desempeño o impacto.',`<button class="btn secondary" data-action="exportPassport">Exportar vista DEMO</button>`)}
      ...
      <section class="passport-hero" style="margin-top:14px"><article class="passport-main"><small>PASSPORT · SANA-DEMO-${lotId}-2026</small><h2>${esc(c.unit?.crop||'Unidad')} · ${esc(c.unit?.name||lotId)}</h2><p>${lotId} · ${c.unit?.area||'—'} ha · Finca La Esperanza · Támesis${plan?` · Plan v${plan.version}`:''}</p><div class="passport-meta"><div><span>Integridad reconstruible</span><strong>${score}%</strong></div><div><span>Pruebas enlazadas</span><strong>${c.evidence.length}</strong></div><div><span>Registros locales</span><strong>${localOnly}</strong></div></div></article><article class="card"><div class="card-head"><div><h2>Brecha prioritaria</h2><p>Lo siguiente que mejora la reconstrucción.</p></div><span class="status ${score>=85?'teal':'warn'}">${score>=85?'ALTA':'MEJORABLE'}</span></div><div class="card-body"><div class="section-note"><strong>${esc(nextGap)}</strong><br>El porcentaje mide completitud DEMO de la cadena; no certifica autenticidad externa ni impacto.</div><div class="gate"><i class="${pendingTasks?'warn':''}">${pendingTasks?'!':'✓'}</i><div><strong>Actividades sin evidencia</strong><p>${pendingTasks} registro(s) requieren prueba o justificación.</p></div><span class="status ${pendingTasks?'warn':'teal'}">${pendingTasks?'ABIERTO':'OK'}</span></div><div class="gate"><i class="${localOnly?'warn':''}">${localOnly?'!':'✓'}</i><div><strong>Eventos LOCAL_ONLY</strong><p>${localOnly} evento(s) aún son sandbox local.</p></div><span class="status ${localOnly?'warn':'teal'}">${localOnly?'NO SINCRONIZADO':'OK'}</span></div></div></article></section>
      ...
      <section class="grid two" style="margin-top:14px"><article class="card"><div class="card-head"><div><h2>Reglas de procedencia</h2><p>Evitan que una historia completa parezca más verificada de lo que realmente está.</p></div></div><div class="card-body"><div class="gate"><i>✓</i><div><strong>DEMO / AGROWAY</strong><p>Dato sintético estructurado dentro del producto.</p></div><span class="status">CONTEXTO</span></div><div class="gate"><i class="warn">!</i><div><strong>LOCAL_ONLY</strong><p>Registro creado en navegador; no implica sincronización o ACK.</p></div><span class="status warn">SANDBOX</span></div><div class="gate"><i class="blocked">×</i><div><strong>VERIFICADO EXTERNO</strong><p>No se atribuye salvo que exista proceso/proveedor independiente.</p></div><span class="status danger">NO ASUMIR</span></div></div></article><article class="card"><div class="card-head"><div><h2>Conexiones operativas</h2><p>Passport sirve como índice, no como silo.</p></div></div><div class="card-body"><div class="quick-grid" style="grid-template-columns:1fr 1fr"><button class="quick" data-view-link="plans"><strong>Abrir plan</strong><span>Versiones, fases y gates.</span></button><button class="quick" data-view-link="advisory"><strong>Abrir acompañamiento</strong><span>Hallazgos y compromisos.</span></button><button class="quick" data-view-link="field"><strong>Abrir campo</strong><span>Actividades y agenda.</span></button><button class="quick" data-view-link="impact"><strong>Abrir impacto</strong><span>Separar evidencia de afirmación.</span></button></div></div></article></section>${footer()}`;
  }

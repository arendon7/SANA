(() => {
  ...
  const GATE_DEFS=[
    ...
    {id:'impact',title:'Impacto y metodología',weight:10,source:'__SANA_IMPACT__',why:'Distinguir indicador trazable, revisión metodológica y verificación externa sin confundirlos.'},
  ...
  function characterizationScore(){
    ...
    const total=keys.reduce((acc,key)=>{const status=local[key]?.status||baseline[key];return acc+(status==='complete'?1:status==='review'?.55:0)},0);
  ...
  function economicsEvidence(){
    ...
    const score=Math.min(100,(configured?30:10)+(api?15:0)+(explicit.length?20:0)+Math.round(supportCoverage*.25)+(explicit.length&&mismatches===0?10:0));
  ...
  function impactEvidence(){
    const summary=window.__SANA_IMPACT__?.summary?.();
    if(!summary)return {score:35,state:'gap',summary:null,detail:'Read-model de impacto no disponible'};
    const reviewBonus=summary.humanReviewed?20:0;
    const externalBonus=summary.externallyVerified?Math.min(15,summary.externallyVerified*5):0;
    const score=Math.min(95,Math.round(summary.overallQuality*.75+reviewBonus+externalBonus));
    const state=summary.externallyVerified>0?'review':summary.humanReviewed?'review':'gap';
    const detail=`${summary.indicators} indicadores · calidad ${summary.overallQuality}% · revisión ${summary.humanReviewed?'humana DEMO':'pendiente'} · ${summary.externallyVerified} verificado(s) externo(s)`;
    return {score,state,summary,detail};
  }
  ...
  function gateData(){
    ...
    return {
      ...
      impact:{score:impact.score,state:impact.state,detail:impact.detail},
      legal:{score:20,state:'blocked',detail:'Fuera de la DEMO: arquitectura jurídica, onboarding, KYC, contratación, pagos/custodia'}
    };
  }

  function overall(gates){const total=GATE_DEFS.reduce((sum,g)=>sum+g.weight,0);return Math.round(GATE_DEFS.reduce((sum,g)=>sum+gates[g.id].score*g.weight,0)/total)}
  ...
  document.addEventListener('click',event=>{
    ...
    const body=`<div class="fields"><label>Necesidad simulada COP<input name="needAmount" type="number" min="0" step="100000" value="${Number(current.needAmount)||48000000}"></label><label>Horizonte<input name="horizon" value="${esc(current.horizon||'10 meses')}"></label><label class="full">Uso previsto<textarea name="useOfFunds" required>${esc(current.useOfFunds||'Capital de trabajo técnico-productivo · insumos, acompañamiento y ejecución del plan DEMO')}</textarea></label><label>Impacto<input value="SE LEE DESDE __SANA_IMPACT__" readonly></label><label>Autoridad<input value="HUMAN_REVIEW_REQUIRED" readonly></label><label class="full">Frontera<input value="LOCAL_ONLY · SIN TRANSACCIONES · SIN CUSTODIA" readonly></label></div>`;
  ...
  document.addEventListener('click',event=>{
    ...
    saveDossier({needAmount:Number(values.needAmount)||0,horizon:values.horizon||'',useOfFunds:values.useOfFunds||'',updatedAt:new Date().toISOString(),localOnly:true});
  },true);
})();

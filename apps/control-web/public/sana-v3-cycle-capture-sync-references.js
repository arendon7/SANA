(() => {
  ...
  const INTEGRITY='SNAPSHOT_CAPTURE_SYNC_REFERENCES_ONLY · CYCLE_SOURCE_SCOPE_ONLY · CYCLE_READ_ONLY · NON_WEIGHTED · CONTENT_MINIMIZED · NO_LIVE_FALLBACK · SOURCE CASE LOT DEFINES CYCLE MEMBERSHIP · TARGET_SCOPE_NEVER_REASSIGNS_CASE · REFERENCE ≠ VERIFIED_SYNC/SOURCE ≠ CYCLE_GATE ≠ DATA_COMPLETENESS ≠ AGRONOMIC/CREDIT/ELIGIBILITY/INVESTMENT_SIGNAL';
  ...
  function forPlan(planId){
    ...
    const d=s.manifest?.captureSyncReferences;if(!d)return {valid:true,state:'NOT_CAPTURED_IN_SNAPSHOT',plan,snapshot:s,cases:[]};
  ...
  function panel(){const s=selected();if(!s.valid)return '';if(s.state!=='CAPTURED')return `<section class="card" style="margin-top:14px"><div class="card-head"><div><p class="kicker">CIERRE · CAPTURE SYNC REFERENCES</p><h2>Granularidad referencial no capturada</h2><p>La ausencia no modifica completeness ni readyForArchive y no se rellena desde Capture/Sync o Data Trust vivos.</p></div><span class="status warn">NO GATE</span></div></section>`;return `<section class="card" style="margin-top:14px"><div class="card-head"><div><p class="kicker">CIERRE · CAPTURE SYNC REFERENCES V153</p><h2>Procedencia referencial del ciclo por scope fuente</h2><p>${esc(s.plan.id)} · ${esc(s.plan.lot)} · solo casos Capture/Sync cuyo lote fuente pertenece al ciclo.</p></div><span class="status ${(s.summary.issues||s.summary.contentLeaks)?'warn':'teal'}">${s.summary.issues||0} ISSUE(S)</span></div><div class="card-body"><div class="grid metrics">${metric('Enlazadas',`${s.summary.linked}/${s.summary.expected}`,'record/ACK/candidatos')}${metric('No canónicas',s.summary.declaredNonCanonical,'solo conteo; valor oculto')}${metric('Targets fuera scope',s.summary.foreignTargets,'visible; no reasigna caso',s.summary.foreignTargets?'warn':'good')}${metric('Impacto en cierre','0','NON_WEIGHTED · no gate','good')}</div><div class="section-note" style="margin-top:12px">SOURCE CASE LOT DEFINES CYCLE MEMBERSHIP · TARGET SCOPE NEVER REASSIGNS CASE · REFERENCE ≠ VERIFIED SYNC/SOURCE ≠ CYCLE GATE · NO DATA COMPLETENESS/AGRONOMIC/CREDIT/ELIGIBILITY/INVESTMENT AUTHORITY.</div></div></section>`}
  ...
  window.__SANA_CYCLE_CAPTURE_SYNC_REFERENCES__=Object.freeze({forPlan,selected,integrity:INTEGRITY});
})();

(() => {
  ...
  function toneForCase(c){return c.semantics.actionLinkIssues||c.referenceIssues?'danger':c.chainCoverage.percent<100?'warn':'teal'}
  ...
  function eventChips(e,c){
    ...
    const ref=PREDECESSOR_KIND[e.eventKind]?eventReference(e,c?.events||null):null;
  ...
  function ledgerPanel(){
    ...
    const observed=explicit.reduce((n,c)=>n+c.semantics.observedPresence,0);const diagnosed=explicit.reduce((n,c)=>n+c.semantics.confirmedDiagnosis,0);const links=explicit.reduce((n,c)=>n+c.activityLinks.length,0);const actions=explicit.reduce((n,c)=>n+c.actions.length,0);const results=explicit.reduce((n,c)=>n+c.results.length,0);const referenceIssues=explicit.reduce((n,c)=>n+c.referenceIssues,0);
    return `<section class="card" style="margin-top:14px"><div class="card-head"><div><p class="kicker">SANIDAD VEGETAL · CADENA V2</p><h2>Ocho transiciones auditables con referencia explícita</h2><p>V124 añade integridad referencial entre eventos V2 sin reescribir la historia V1. Compartir caseId ya no sustituye la referencia al predecesor.</p></div><button class="btn primary" data-health-new-case>Abrir caso sanitario</button></div><div class="card-body"><div class="grid metrics">${metric('Casos',explicit.length,'proyección V2 sobre ledger V1')}${metric('Presencias observadas',observed,'no inferidas desde alertas')}${metric('Diagnósticos humanos',diagnosed,'confirmaciones explícitas')}${metric('Vínculos Activity V2',links,'relación ≠ ejecución')}${metric('Resultados V2',results,'resultado ≠ causalidad')}${metric('Issues referencia V2',referenceIssues,'solo eventos V2; legacy no penaliza')}</div><div class="section-note" style="margin-top:12px"><strong>Contrato V124:</strong> CASE_MEMBERSHIP ≠ PREDECESSOR_REFERENCE · REFERENCE ≠ CAUSALITY. Los eventos V1 sin referencia explícita permanecen históricos y no se marcan como inválidos.</div></div></section><section class="grid two" style="margin-top:14px">${explicit.map(caseCard).join('')||'<div class="empty">Sin casos V2.</div>'}</section>${legacyPanel(legacy)}`;
  }
  ...
  window.__SANA_PHYTOSANITARY_LEDGER__=Object.freeze({schema:SCHEMA,projection:PROJECTION,projectionVersion:'V2',stages:STAGES.map(([id,label])=>({id,label})),legacyStages:LEGACY_STAGES.map(([id,label])=>({id,label})),predecessorKinds:PREDECESSOR_KIND,events:explicitEvents,cases,forCase:caseFor,forLot,legacy:legacySummaries,eventReference,integrity:'LEGACY_INCIDENT_SUMMARY ≠ OBSERVED_PRESENCE ≠ DIAGNOSIS ≠ TREATMENT ≠ EFFICACY · OBSERVATION ≠ DIAGNOSIS · DIAGNOSIS ≠ RECOMMENDATION · RECOMMENDATION ≠ EXECUTION · ACTIVITY_LINK_EVENT ≠ ACTION · FOLLOW_UP ≠ RESULT · RESULT ≠ CAUSAL_ATTRIBUTION · EMBEDDED_V1_RELATION ≠ V2_STAGE · CASE_MEMBERSHIP ≠ PREDECESSOR_REFERENCE · REFERENCE ≠ CAUSALITY'});
})();

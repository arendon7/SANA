(() => {
  ...
  function envelopeWithAttestation(envelope,sourceState){
    if(sourceState.state!=='AVAILABLE')return deepFreeze({...envelope,linkDiagnostic:{state:'SOURCE_UNAVAILABLE',sourceState:sourceState.state,reason:sourceState.reason,ruleId:RULE.id,candidateCount:0,validCandidateCount:0,conflictReasons:Object.freeze([])},authority:AUTHORITY,integrity:INTEGRITY});
    const candidates=sourceState.records.filter(r=>r&&typeof r==='object'&&text(r.claimId)===envelope.claimId);
    if(!candidates.length)return deepFreeze({...envelope,linkDiagnostic:{state:'NO_EXACT_CANDIDATE',sourceState:'AVAILABLE',reason:null,ruleId:RULE.id,candidateCount:0,validCandidateCount:0,conflictReasons:Object.freeze([])},authority:AUTHORITY,integrity:INTEGRITY});
    ...
    if(conflictReasons.length||valid.length!==candidates.length){
      return deepFreeze({...envelope,linkDiagnostic:{state:'CLAIM_ATTESTATION_CANDIDATE_CONFLICT',sourceState:'AVAILABLE',reason:'EXACT_CLAIM_CANDIDATE_CONTRADICTION',ruleId:RULE.id,candidateCount:candidates.length,validCandidateCount:valid.length,conflictReasons:Object.freeze(conflictReasons)},authority:AUTHORITY,integrity:INTEGRITY});
    }
  ...
  function build(host,options={}){
    ...
    return deepFreeze({schema:SCHEMA,version:VERSION,parent:PARENT,parentSha:PARENT_SHA,scope:parent.scope,envelopes,summary:summarize(envelopes,sourceState),sourceContract:{globalName:SOURCE_GLOBAL,schema:SOURCE_SCHEMA,state:sourceState.state,reason:sourceState.reason,recordCount:sourceState.records.length},audit:{linkRules:LINK_RULES,ruleCount:1,heuristicLinking:false,sourceStates:SOURCE_STATES,conflictPolicy:RULE.conflictPolicy,provenanceGap:parent.audit?.provenanceGap||null},authority:AUTHORITY,provenance:{parent:PARENT,parentSha:PARENT_SHA,parentSchema:parent.schema,parentVersion:parent.version,sourceSchema:SOURCE_SCHEMA,linkRule:RULE.id,linkPolicy:'EXACT_CONTRACTED_LINKS_ONLY'},integrity:INTEGRITY});
  }

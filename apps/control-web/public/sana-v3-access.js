(() => {
  ...
  const allViews=['home','territory','characterization','material','plans','field','phenology','nutrition','health','inventory','forecast','circularity','results','economics','team','iot','reports','advisory','passport','intelligence','impact','capital','control'];
  const viewPolicy={
    admin:new Set(allViews),
    technical:new Set(['home','territory','characterization','material','plans','field','phenology','nutrition','health','inventory','forecast','circularity','results','economics','iot','reports','advisory','passport','intelligence','impact','capital']),
    producer:new Set(['home','territory','characterization','plans','field','phenology','nutrition','health','inventory','forecast','circularity','results','economics','iot','reports','advisory','passport','intelligence','impact','capital']),
  ...
  const actionPolicy={
    admin:['*'],
    technical:['fieldRecord','quickField','task-toggle','phenology','nutrition','health','inventory','material','sensor','visit','structured-visit','agronomist-case','input-forecast-adjustment','plan','plan-review','harvest-result','circularity-residue','report','reportOpen','methodology','impact-methodology','knowledge','exportPassport','queue-review'],
    producer:['fieldRecord','quickField','task-toggle','phenology','nutrition','health','inventory','structured-visit','agronomist-case','input-forecast-adjustment','harvest-result','circularity-residue','cost','methodology','reportOpen','exportPassport','queue-review','characterization:*','capital-dossier'],
  ...
  const selectorActions=[
    ['[data-task]','task-toggle'],['[data-queue]','queue-review'],['[data-field-quick]','quickField'],['[data-character-section]','characterization:*'],['[data-plan-review]','plan-review'],['[data-plan-transition]','plan-review'],['[data-structured-visit]','structured-visit'],['[data-agronomist-case]','agronomist-case'],['[data-forecast-adjust]','input-forecast-adjustment'],['[data-harvest-result]','harvest-result'],['[data-circularity-residue]','circularity-residue'],['[data-capital-config]','capital-dossier'],['[data-economics-cost]','cost'],['[data-impact-methodology]','impact-methodology']
  ];
  ...
  document.addEventListener('click',event=>{
    const nav=event.target.closest('[data-view],[data-view-link]');if(nav){const view=nav.dataset.view||nav.dataset.viewLink;if(view&&!canView(view)){event.preventDefault();event.stopImmediatePropagation();deny('Este rol no tiene acceso a esa vista en la DEMO.');return}}
    for(const [selector,type] of selectorActions){if(event.target.closest(selector)&&!canAction(type)){event.preventDefault();event.stopImmediatePropagation();deny('El rol actual es de lectura o tiene un alcance operativo más limitado.');return}}
  },true);

  const observer=new MutationObserver(()=>{enforceNav();enforceRoleLabel();wrapRuntime()});observer.observe(document.documentElement,{subtree:true,childList:true});
  const mount=()=>{enforceNav();enforceRoleLabel();wrapRuntime()};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',mount,{once:true});else mount();setTimeout(wrapRuntime,0);

(() => {
  ...
  function inventory(){
    ...
    return `${head('AGROWAY · INVENTARIOS Y ESPACIOS','Stock trazable, capacidad visible y brechas conectadas al plan.','El inventario combina existencia base DEMO, movimientos por identidad, mínimos operativos, espacios/tanques y demanda proyectada. Una alerta informa; no genera compra, reposición, despacho, dosis ni aplicación automática.',`<button class="btn primary" data-inventory-movement>Registrar movimiento DEMO</button>`)}
      <section class="grid metrics">${metric('Ítems modelados',list.length,'agroinsumos · equipos · espacios','good')}${metric('Movimientos propios',moves.length,moves.length?'persisten por identidad':'aún sin movimientos',moves.length?'good':'')}${metric('Brechas / mínimos',low.length,low.length?'requieren revisión humana':'sin señal crítica',low.length?'warn':'good')}${metric('Espacios y tanques',SPACES.length,'capacidad y restricciones DEMO','good')}</section>
      ...
      <section class="grid two" style="margin-top:14px"><article class="card"><div class="card-head"><div><h2>Espacios, tanques y capacidades</h2><p>Capacidad operativa visible sin control automático de equipos.</p></div></div><div class="card-body">${SPACES.map(s=>`<div class="gate"><i>${Math.round(s.use/10)}</i><div><strong>${esc(s.name)} · ${esc(s.capacity)}</strong><p>${esc(s.type)} · ${esc(s.restriction)}</p>${progress(s.use,s.use>85?'warn':'teal')}</div><span class="status ${s.use>85?'warn':'teal'}">${s.use}% USO</span></div>`).join('')}</div></article><article class="card"><div class="card-head"><div><h2>Frontera de decisión</h2><p>Inventario ≠ compras ni dosificación.</p></div></div><div class="card-body"><div class="gate"><i class="warn">!</i><div><strong>Brecha o mínimo</strong><p>Genera señal para revisión física y planificación.</p></div><span class="status warn">REVISAR</span></div><div class="gate"><i class="blocked">×</i><div><strong>Orden de compra / pago</strong><p>No existe desde esta vista.</p></div><span class="status danger">BLOQUEADO</span></div><div class="gate"><i class="blocked">×</i><div><strong>Despacho / dosis automática</strong><p>Requiere proceso y autoridad separados.</p></div><span class="status danger">HUMAN_ONLY</span></div></div></article></section>
      <article class="card" style="margin-top:14px"><div class="card-head"><div><h2>Bitácora de movimientos</h2><p>Entrada, salida o ajuste con responsable, destino y evidencia.</p></div><span class="status">LOCAL/NUBE DEMO</span></div><div class="card-body">${moves.length?moves.map(m=>`<div class="row"><span class="dot ${m.movement==='SALIDA'?'warn':''}"></span><div class="copy"><strong>${esc(m.movement)} · ${esc(DEMO.inventory.find(i=>i.id===m.itemId)?.name||m.itemId)}</strong><span>${esc(m.lot||'Almacén')} · ${esc(m.detail||'Sin nota')} · evidencia ${esc(m.evidence||'—')}</span></div><div class="meta">${esc(m.qty||'0')}<br>${esc(m.owner||'Sin responsable')}</div></div>`).join(''):'<div class="empty">Aún no hay movimientos creados por esta identidad.</div>'}</div></article>${footer()}`;
  }
  views.inventory=inventory;
  window.__SANA_INVENTORY__=Object.freeze({rows:()=>rows().map(i=>({...i,current:{...i.current}})),movements:()=>movements().map(m=>({...m})),spaces:()=>SPACES.map(s=>({...s}))});

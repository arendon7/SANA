...
assert.equal(api.reference(E.find(e=>e.id==='I1'),'S1').status,'LINKED');
assert.equal(api.reference(E.find(e=>e.id==='IMISS'),'NOPE').status,'MISSING_TARGET');
assert.equal(api.reference(E.find(e=>e.id==='IKIND'),'E1').status,'KIND_MISMATCH');
assert.equal(api.reference(E.find(e=>e.id==='ICROSS'),'S2').status,'CROSS_LOT_REFERENCE');
assert.equal(api.reference(E.find(e=>e.id==='IFWD'),'MF').status,'FORWARD_REFERENCE');
...
assert.equal(api.reference(E.find(e=>e.id==='ILEG'),'S1').status,'LEGACY_REFERENCE_NOT_CAPTURED');

...
{const store=new Store(),db=new DB({failSubmission:1}),service=new mod.ReadinessEvidenceIngestionService(db,store,new Scanner(),policy);const retryInput=input('application/pdf',media['application/pdf'],'ingest-retry-0000001');await deny(()=>service.ingestAndSubmit(retryInput),'SIMULATED_SUBMISSION_FAILURE');assert(db.receipts.size===1&&store.deletes.length===0,'TRANSITION_FAILURE_PRESERVES_RECEIPT');const first=[...db.receipts.values()][0];const result=await service.ingestAndSubmit(retryInput);assert(result.receipt.receiptId===first.receiptId,'DETERMINISTIC_RECEIPT_RETRY');assert(db.receipts.size===1&&db.current==='EVIDENCE_SUBMITTED','RETRY_SUBMITS_EXISTING_RECEIPT')}
{const db1=new DB(),db2=new DB(),store1=new Store(),store2=new Store(),scanner=new Scanner(),service1=new mod.ReadinessEvidenceIngestionService(db1,store1,scanner,policy),service2=new mod.ReadinessEvidenceIngestionService(db2,store2,scanner,policy);const i=input('application/pdf',media['application/pdf'],'ingest-deterministic01');const a=await service1.ingestAndSubmit(i),b=await service2.ingestAndSubmit(i);assert(a.receipt.receiptId===b.receipt.receiptId&&a.receipt.evidenceRef===b.receipt.evidenceRef,'DETERMINISTIC_IDENTITIES')}

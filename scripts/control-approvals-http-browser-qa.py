...
try:
  with sync_playwright() as p:
    ...
    for label,w,h in [('desktop',1440,900),('mobile',390,844)]:
      ...
      page.locator('[data-id="ap-invest-yar"]').click();page.locator('#approver').select_option('fin-sb-001');page.locator('#approvalNote').fill('Intento de autoaprobación controlado.');page.locator('#approveBtn').click();page.wait_for_timeout(100);ck(f'{label}:separation-of-duties','HIGH_RISK_SEPARATION_OF_DUTIES_REQUIRED' in page.locator('#toast').inner_text());store=page.evaluate("JSON.parse(localStorage.getItem('agroway.control.demo.approvals.v1')||'[]')");inv=next(x for x in store if x['id']=='ap-invest-yar');ck(f'{label}:self-approval-not-recorded',len(inv['approvalRecords'])==0)

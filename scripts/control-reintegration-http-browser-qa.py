...
def run_view(browser,label,viewport):
    ...
    check(f'{label}:home-capital-nav',page.locator('.rail-nav a[href="/control/capital"]').count()==1)
    ...
    check(f'{label}:project-readiness-link',page.locator('.alpha8-project-actions a[href="/control/capital"]').count()==1)
    ...
    check(f'{label}:exception-capital-readiness-link',page.locator('.flow-capital-link[href="/control/capital"]').count()==1)
    exception_overflow=page.evaluate("document.documentElement.scrollWidth<=document.documentElement.clientWidth+1")
    check(f'{label}:exception-no-overflow',exception_overflow)
    page.screenshot(path=str(OUT/f'{label}.png'),full_page=True)

    page.locator('.flow-capital-link').click();page.wait_for_url('**/control/capital')
    check(f'{label}:capital-queue-route',page.url.rstrip('/')==BASE+'/control/capital')
    check(f'{label}:capital-queue-title',page.locator('h1').first.text_content().strip()=='Proyectos en preparación')
    check(f'{label}:capital-synthetic-fixture','FIXTURE_SYNTHETIC' in page.locator('body').text_content())
    check(f'{label}:capital-no-financing-approval','CAPITAL_READY ≠ financiación aprobada' in page.locator('body').text_content())
    check(f'{label}:capital-three-projects',page.locator('.project-row:not(.header)').count()==3,page.locator('.project-row:not(.header)').count())
    page.locator('.project-row:not(.header)').first.click();page.wait_for_url('**/control/capital/projects/hass-san-miguel')
    check(f'{label}:capital-project-route',page.url.endswith('/control/capital/projects/hass-san-miguel'))
    check(f'{label}:capital-initial-not-ready','NOT CAPITAL READY' in page.locator('.decision-card').text_content())
    check(f'{label}:capital-three-blockers',page.locator('.gap-card').count()==3,page.locator('.gap-card').count())
    check(f'{label}:capital-nine-gates',page.locator('.gate-item').count()==9,page.locator('.gate-item').count())
    check(f'{label}:capital-two-conditions',page.locator('.gate-state.condition').count()==2,page.locator('.gate-state.condition').count())
    check(f'{label}:capital-nine-risk-dimensions',page.locator('.risk-item').count()==9,page.locator('.risk-item').count())
    check(f'{label}:capital-read-only-no-forms',page.locator('form').count()==0)
    check(f'{label}:capital-read-only-no-buttons',page.locator('button').count()==0)
    check(f'{label}:capital-no-money-movement','no mueve dinero' in page.locator('.method').text_content().lower())
    capital_overflow=page.evaluate("document.documentElement.scrollWidth<=document.documentElement.clientWidth+1")
    check(f'{label}:capital-no-overflow',capital_overflow)
    page.screenshot(path=str(OUT/f'{label}-capital.png'),full_page=True)
...
report={'status':'PASS' if not failed else 'FAIL','checks':len(results),'failed':failed,'results':results,'transport':'HTTP_REAL','sourceRecovery':'RECOVERED_EXACT_PR9_BLOBS_PLUS_CAPITAL_UX1_SYNTHETIC_FIXTURE','sourceCommit':'3d58de6cbbac3946fab1fc255a28d08d61166b59','authority':'HUMAN_ONLY','aiAuthority':'ADVISORY_ONLY','executionState':'NOT_EXECUTED','canonicalMutation':False,'capitalUx1':'READ_ONLY','d10':'PENDING'}

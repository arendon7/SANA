...
for(const file of files){
  ...
  const identity=[doc.id,doc.screen,doc.screenId].find(value=>typeof value==='string'&&value.trim())?.trim()||'';
  ...
  const events=doc.canonicalDomainEventsAdded??doc.domainDelta?.canonicalDomainEventsAdded??doc.domainEventDelta;
  const workspaces=doc.workspacesAdded??doc.domainDelta?.workspacesAdded??doc.workspaceDelta;

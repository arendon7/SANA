...
const meta=['CR-GOOD','CR-BAD'].map((caseId,i)=>({id:`M${i}`,type:'capital-review-reference-meta',values:{sourceSchema:base.schema,caseId,referenceVersion:'V160'}}));
const sandbox={window:{
  __SANA_CAPITAL_REVIEW__:base,
  __SANA_CAPITAL_GOVERNANCE__:{schema:'SANA_CAPITAL_GOVERNANCE_LEDGER_V1',cases:()=>governanceCases.map(clone)},
...
assert.equal(good.referenceState,'CAPTURED_V160');
...
assert.equal(summary.referenceExpected,6);
assert.equal(summary.referenceLinked,3);
...
console.log('capital review references V160: ok');

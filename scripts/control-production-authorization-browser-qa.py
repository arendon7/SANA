...
try:
    ...
    with sync_playwright() as p:
        ...
        for label,viewport in [('desktop',{'width':1440,'height':900}),('mobile',{'width':390,'height':844})]:
            ...
            authenticate_demo_admin(page,BASE+'/control/authorization');page.goto(BASE+'/control/authorization',wait_until='networkidle');check(f'{label}:route',page.url.endswith('/control/authorization'),page.url);check(f'{label}:title',page.locator('h1').inner_text()=='Production Authorization Boundary');body=page.locator('body').inner_text()

...
async function copyFile(from,to,logical){const body=await fs.readFile(from);await fs.mkdir(path.dirname(to),{recursive:true});await fs.writeFile(to,body);entries.push({path:logical,bytes:body.length,sha256:digest(body)})}

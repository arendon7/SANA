...
function walk(dir,relative=''){
  for(const name of fs.readdirSync(dir).sort()){
    ...
    if(stat.isDirectory())walk(full,rel);else if(rel!=='MANIFEST.json'&&rel!=='MANIFEST.sha256'){
...
const manifestSha256=sha256(fs.readFileSync(path.join(out,'MANIFEST.json')));
write('MANIFEST.sha256',`${manifestSha256}\n`);
console.log(JSON.stringify({status:'PASS',release:manifest.release,output:path.relative(root,out),files:inventory.length,manifestSha256,reviewBundleSha256:manifestSha256,reviewBundleDigestSource:'MANIFEST.json',commissioningReady:true,productionReady:false,blockers:blockers.length},null,2));

...
const readinessTruth=cfg.authority.readinessOrchestratorImplemented===true
  ? 'The production readiness orchestrator is deterministic server-side certification logic. It aggregates only read-only IdP/JWKS, PostgreSQL and External ACK connectivity evidence plus exact-release-bound HUMAN_ONLY D10 evidence. It has no write port, ACK transport creation, ProductionSession creation or browser activation capability. READY_FOR_EXPLICIT_ACTIVATION_REVIEW still means productionExecutionAvailable=false and does not activate production.'
  : '';

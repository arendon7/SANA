...
const gh=value=>String(value).replace(/%/g,'%25').replace(/\r/g,'%0D').replace(/\n/g,'%0A');
...
async function main(){
  ...
  const config=resolveProductionPostgresConfig(env);const factory=await createPinnedPostgresJsPoolFactory();check('driver:version',factory.driver.version==='3.4.9');const pool=factory.create(config);
  let client;
  try{
  ...
  const wiring=await createProductionPostgresWiringWithPinnedDriver(env);try{
    check('wiring:configured',wiring.state==='CONFIGURED_NOT_CONNECTED');assert.throws(()=>wiring.createCanonicalUnitOfWorkAfterConnectivityCertification(),/PRODUCTION_POSTGRES_CONNECTIVITY_CERTIFICATION_REQUIRED/);check('wiring:fail-closed-before-probe',true);const evidence=await wiring.verifyConnectivity();check('wiring:read-only-probe',evidence.state==='CONNECTED_READ_ONLY_PROBE'&&evidence.canonicalWriteExecuted===false);check('wiring:no-production-write',wiring.canonicalWriteExecuted===false);
  }finally{await wiring.closeDriver().catch(()=>undefined)}
...
try{await main()}catch(error){
  ...
  console.error(`::error title=ALPHA14_POSTGRES_JS_TLS_FAILURE::${gh(details)}`);

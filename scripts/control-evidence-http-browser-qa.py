...
try:
  with sync_playwright() as p:
    ...
    for label,w,h in [('desktop',1440,900),('mobile',390,844)]:
      ...
      sw=page.evaluate("""async()=>{if(!('serviceWorker' in navigator))return {ok:false};const reg=await navigator.serviceWorker.ready;return {ok:!!reg.active,url:reg.active?.scriptURL||''}}""");ck(f'{label}:service-worker',sw['ok'] and sw['url'].endswith('/service-worker.js'),sw)

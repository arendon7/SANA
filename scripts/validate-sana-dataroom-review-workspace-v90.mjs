...
const window={
  ...
  __SANA_DATAROOM_REVIEW_CASE__:{cases:()=>[full('CASE-T')]},
...
const api=window.__SANA_DATAROOM_REVIEW_WORKSPACE__;assert(api);const s=api.state();assert.equal(s.chains.length,1);const f=api.readFocus();assert.equal(f.capital,'CAP-T');assert.equal(f.lot,'T');assert.equal(f.focus,'ALL');assert.equal(f.stage,'CASE');const e=api.selectedEntry(s.chains,f);assert(e);assert.equal(e.id,'CASE-T');assert.equal(e.stage,'CASE');assert.equal(e.sourceSchema,'SCHEMA_CASE-T');assert.equal(e.eventCount,2);assert.equal(e.firstObservedAt,'2026-08-18T13:00:00-05:00');assert.equal(e.lastObservedAt,'2026-08-18T14:00:00-05:00');assert.equal(api.selectedEntry(s.chains,{capital:'ALL',lot:'ALL',focus:'ALL',stage:'ALL'}),null);for(const k of ['riskScores','dueDiligenceApprovals','investmentDecisions','externalActions'])assert.equal(s.summary[k],0);

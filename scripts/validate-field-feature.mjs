...
if(spec.config){const configPath=path.join('config/design/screens',spec.config);check('design:contract-present',fs.existsSync(configPath),configPath);if(fs.existsSync(configPath)){const doc=JSON.parse(fs.readFileSync(configPath,'utf8'));const identity=doc.id??doc.screen;check('design:identity',typeof identity==='string'&&identity.length>0);const reviewState=doc.status??doc.gates?.D10_humanProductApproval??doc.d10??doc.humanProductApproval;check('design:review-state',reviewState!==undefined);if(doc.requiredViewports)check('design:viewports',Array.isArray(doc.requiredViewports)&&doc.requiredViewports.length>0);if(doc.gates?.D10_humanProductApproval)check('design:d10-valid',['PENDING','PASS'].includes(doc.gates.D10_humanProductApproval));if(doc.d10)check('design:d10-valid',['PENDING','PASS'].includes(doc.d10));if(doc.humanProductApproval)check('design:human-approval-valid',['PENDING','PASS','APPROVED','NOT_REQUIRED'].includes(doc.humanProductApproval));}}
